...
test("RestoreManager 镜像恢复会清理备份中不存在的空目录", async () => {
  const manager = new RestoreManager();
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "ccm-restore-mirror-empty-dir-"));
  const promptsDir = path.join(tempDir, "prompts");
  const extraDir = path.join(promptsDir, "old-team");
  const extraNestedDir = path.join(extraDir, "nested");
  const extraFile = path.join(extraNestedDir, "stale.md");
  const restoredFile = path.join(promptsDir, "team", "welcome.md");

  await fs.mkdir(path.dirname(restoredFile), { recursive: true });
  await fs.mkdir(extraNestedDir, { recursive: true });
  await fs.writeFile(extraFile, "stale");
  ...
  try {
    ...
    // 多余文件和空目录都应被清理
    await assert.rejects(fs.access(extraFile));
    await assert.rejects(fs.access(extraNestedDir));
    await assert.rejects(fs.access(extraDir));
  } finally {
    await fs.rm(tempDir, { recursive: true, force: true });
  }
});
